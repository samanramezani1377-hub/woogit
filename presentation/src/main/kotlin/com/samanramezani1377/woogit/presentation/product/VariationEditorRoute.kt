...
internal fun VariationEditorRoute(dependencies:V1PresentationDependencies,storeId:StoreId,productId:String,variationId:String?,onBack:()->Unit,onSaved:()->Unit){
    ...
    when{
        variationId==null || state is FeatureUiState.Success->VariationFormScreen(form,{form=form.copy(sku=it)},{form=form.copy(price=it)},{form=form.copy(sale=it)},{form=form.copy(stock=it)},{form=form.copy(attributes=it)},{val attrs=form.attributes.split('|').mapNotNull{raw->val p=raw.split(':',limit=2);if(p.size==2&&p[0].isNotBlank()&&p[1].isNotBlank())VariationAttribute(p[0].trim(),p[1].trim())else null};val v=Variation(EntityId(variationId?:"new"),EntityId(productId),attrs,Pricing(form.price.ifBlank{null},form.sale.ifBlank{null},form.sale.isNotBlank()),form.stock.toDoubleOrNull()?.let{Stock(it,if(it>0)StockStatus.IN_STOCK else StockStatus.OUT_OF_STOCK,true)},form.sku.ifBlank{null},null);vm.save(storeId,EntityId(productId),EntityId(variationId?:"new"),v,variationId==null,onSaved)},onBack),
