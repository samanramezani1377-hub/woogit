...
internal class DeepSeekProvider(context: Context) : AiProvider {
    ...
    override suspend fun stream(messages: JSONArray, tools: JSONArray, onEvent: suspend (AiStreamEvent) -> Unit): JSONObject = requestStream(messages, tools, onEvent)
    ...
    private suspend fun requestStream(messages: JSONArray, tools: JSONArray, onEvent: suspend (AiStreamEvent) -> Unit): JSONObject {
        val key = apiKey.trim()
    ...
    private suspend fun readSse(connection: HttpURLConnection, onEvent: suspend (AiStreamEvent) -> Unit): JSONObject {
        ...
        connection.inputStream.bufferedReader().useLines { lines ->
            lines.forEach { line ->
                ...
                if (raw == "[DONE]" || raw.isBlank()) return@forEach
                val delta = runCatching { JSONObject(raw).optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("delta") }.getOrNull() ?: return@forEach
                ...
                val streamedCalls = delta.optJSONArray("tool_calls") ?: return@forEach
                for (i in 0 until streamedCalls.length()) {
                    ...
                    if (fn.has("name")) { current.put("name", current.optString("name") + fn.optString("name")); onEvent(AiStreamEvent.ToolCall(fn.optString("name"))) }
