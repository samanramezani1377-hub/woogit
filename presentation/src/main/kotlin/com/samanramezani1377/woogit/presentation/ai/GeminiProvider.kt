...
internal class GeminiProvider(context: Context) : AiProvider {
    ...
    private fun toOpenAiResponse(response: JSONObject): JSONObject {
        ...
        val text = StringBuilder(); val calls = JSONArray(); val images = mutableListOf<String>(); var textSignature: String? = null
        for (i in 0 until parts.length()) { val part = parts.optJSONObject(i) ?: continue; val signature = part.optString("thoughtSignature").takeIf { it.isNotBlank() }; if (signature != null) textSignature = signature; part.optString("text", "").takeIf { it.isNotEmpty() }?.let(text::append); val inline = part.optJSONObject("inlineData"); if (inline != null) AiOutputImageCodec.markerFromBase64("generated-image", inline.optString("mimeType").ifBlank { "image/png" }, inline.optString("data")).takeIf { it != null }?.let(images::add); val fc = part.optJSONObject("functionCall") ?: continue; val name = fc.optString("name"); if (name.isBlank()) continue; val args = fc.optJSONObject("args") ?: JSONObject(); val id = fc.optString("id").ifBlank { syntheticCallId(name, args.toString()) }; val function = JSONObject().put("name", name).put("arguments", args.toString()); signature?.let { function.put("thought_signature", it) }; calls.put(JSONObject().put("id", id).put("type", "function").put("function", function)) }
        val message = JSONObject().put("role", "assistant").put("content", text.toString() + images.joinToString("")); if (textSignature != null && calls.length() == 0) message.put("thought_signature", textSignature); if (calls.length() > 0) message.put("tool_calls", calls); return JSONObject().put("choices", JSONArray().put(JSONObject().put("message", message)))
    }
