...
internal class AiAgent(
    ...
    suspend fun run(
        ...
        val pendingSingle = if (pendingBatch == null) {
            confirmationToken?.let { pending[it] }
        } else {
            null
        }
        val activeAttachments = if (attachments.isNotEmpty()) {
            ...
            pendingSingle?.attachments.orEmpty()
                .ifEmpty { pendingBatch?.items?.firstOrNull()?.attachments.orEmpty() }
        }
        ...
        val working = JSONArray().apply {
            put(
                JSONObject()
                    ...
                    .put(
                        "content",
                        AiAgentPrompt.SYSTEM_PROMPT + attachmentContext + memory.contextText(),
                    ),
            )
        ...
        if (pendingBatch != null) {
            resultAttachments = executeBatchConfirmation(
                pendingBatch,
                batchSelection,
                conversationId,
                working,
            ...
            val action = pending.remove(confirmationToken)
                ?: throw IllegalStateException(
                    "عملیات در انتظار تأیید پیدا نشد. دوباره درخواست را ارسال کنید.",
                )
            onEvent(AiStreamEvent.Status("در حال اجرای عملیات تأییدشده..."))
            working.put(
                AiAgentTools.assistantToolCall(
                    action.callId,
                    action.name,
                    action.arguments,
                    action.thoughtSignature,
                ),
            )
            ...
            onEvent(
                AiStreamEvent.ToolResult(
                    AiAgentTools.label(action.name),
                    summarize(result),
                ),
            )
            if (AiAgentTools.isWrite(action.name)) {
        ...
        repeat(MAX_STEPS) { step ->
            ...
            onEvent(
                AiStreamEvent.Status(
                    if (step == 0) {
                        ...
                        "در حال بررسی نتیجه مرحله قبل..."
                    },
                ),
            )
            val response = provider.stream(
                working,
                AiAgentTools.definitions(),
                attachmentsForNextRequest,
                onEvent,
            )
            ...
            if (calls == null || calls.length() == 0) {
                ...
                workingMemory.complete(
                    conversationId,
                    text.ifBlank { "کار Agent با موفقیت به پایان رسید." },
                )
                return AgentReply(
                    text = text,
            ...
            if (calls.length() > 1) {
                for (i in 0 until calls.length()) {
                    ...
                    val thoughtSignature = fn.optString("thought_signature")
                    ...
                    if (AiAgentTools.isWrite(name)) {
                        writeCalls += PendingBatchItem(
                            sha256("$callId:$name:$arguments").take(20),
                            name,
                            arguments,
                            callId,
                            thoughtSignature,
                            batchLabel(name, arguments),
                            attachments,
                        )
                    }
                }
            ...
            if (writeCalls.size == calls.length() && writeCalls.size > 1) {
                ...
                val batch = PendingBatchConfirmation(
                    token,
                    "تأیید عملیات گروهی",
                    "AI برای یک درخواست چند تغییر مستقل آماده کرده است. موارد موردنظر را انتخاب کنید.",
                    writeCalls,
                )
                ...
                writeCalls.forEach { item ->
                    items.put(
                        JSONObject()
                            .put("itemId", item.id)
                            .put("tool", item.name)
                            .put("arguments", item.arguments.take(MAX_OPERATION_ARGUMENTS))
                            .put("label", item.label)
                            .put("status", "PENDING_CONFIRMATION"),
                    )
                }
                workingMemory.createBatch(
                    conversationId,
                    token,
                    latestUserText.ifBlank { "Batch operation" },
                    items,
                )
            ...
            for (i in 0 until calls.length()) {
                ...
                if (AiAgentTools.isWorkingMemory(name)) {
                    val result = workingMemory
                        .tool(conversationId, JSONObject(arguments))
                        .toString()
                    working.put(
                        JSONObject()
                            .put("role", "tool")
    ...
    private suspend fun executeBatchConfirmation(
        ...
        batch.items.forEach { item ->
            ...
            working.put(
                JSONObject()
                    .put("role", "tool")
                    .put("tool_call_id", item.callId)
                    .put("content", result),
            )
            onEvent(
                AiStreamEvent.ToolResult(
                    AiAgentTools.label(item.name),
                    summarize(result),
                ),
            )
            val json = runCatching { JSONObject(result) }
...
$outcome""".trimIndent(),
    ...
    private fun latestUserText(messages: List<Pair<String, String>>): String =
        messages.asReversed()
            .firstOrNull { it.first.equals("user", ignoreCase = true) }
            ?.second
            ?.trim()
            .orEmpty()
    ...
    private fun statusReply(conversationId: String): AgentReply {
        ...
        val checkpointStatus = state
            .optJSONObject("checkpoint")
            ?.optString("status")
            .orEmpty()
        val checkpointText = if (checkpointStatus == "in_flight") {
            " آخرین عملیات هنوز نتیجه نهاییِ ذخیره‌شده ندارد و قبل از تکرار باید وضعیت واقعی فروشگاه بررسی شود."
        } else {
            ""
        }
        val progressText = if (total > 0) {
            "$completed از $total مورد انجام شده و $remaining مورد باقی مانده"
        } else if (remaining > 0) {
            "$remaining مورد باقی مانده"
        } else {
            "مقدار دقیق باقی‌مانده ثبت نشده است"
        }
        return AgentReply(
            text = when {
                status == "completed" && remaining <= 0 && summary.isNotBlank() ->
                    "آخرین اجرای ثبت‌شده کامل شده است. $summary$checkpointText"

                status == "active" ->
                    "کار در حال پیگیری است: $progressText.${if (summary.isNotBlank()) " آخرین جمع‌بندی: $summary" else ""}$checkpointText"

                summary.isNotBlank() ->
                    "آخرین وضعیت: $summary؛ $progressText.$checkpointText"

                else ->
                    "آخرین وضعیت Working Memory: $progressText.$checkpointText"
            },
        )
    }

    private fun activateWorkingMemory(
        conversationId: String,
        messages: List<Pair<String, String>>,
        working: JSONArray,
    ) {
        val existing = workingMemory.read(conversationId)
        val state = workingMemory.ensure(
            conversationId,
            latestUserText(messages).ifBlank { "Agent task" },
        )

        if (existing?.optString("status") == "active") {
            val inFlight = existing
                .optJSONObject("checkpoint")
                ?.optString("status") == "in_flight"
...
$state""".trimIndent(),
        ...
        working.put(
            JSONObject()
                ...
                .put(
                    "content",
                    "Working Memory برای این اجرای جاری فعال شد. برای کارهای چندمرحله‌ای استفاده از آن اجباری است؛ progress و checkpoint را به‌صورت delta ثبت کن و عملیات موفق را تکرار نکن.",
                ),
        )
    }

    private fun beginInFlightOperation(
        conversationId: String,
        name: String,
        arguments: String,
    ...
    private suspend fun generateMandatoryFinalResponse(
        ...
        val context = state?.let {
            JSONObject()
                ...
                .put("batches", it.optJSONArray("batches") ?: JSONArray())
...
$context""".trimIndent(),
    ...
    private fun mandatoryFallback(conversationId: String): String {
        ...
        val inFlight = state
            ?.optJSONObject("checkpoint")
            ?.optString("status") == "in_flight"
        val suffix = if (inFlight) {
            " آخرین عملیات قبل از تکمیل checkpoint متوقف شده و قبل از تکرار باید وضعیت واقعی فروشگاه بررسی شود."
        } else {
            ""
        }
        return if (completed >= 0 && total > 0) {
            "بخشی از کار انجام شد و اجرای ابزارها به سقف این مرحله رسید: $completed از $total مورد تکمیل شده است. وضعیت کار ذخیره شده و می‌توان با گفتن «ادامه بده» از همین نقطه ادامه داد.$suffix"
        } else {
            "بخشی از کار انجام شد، اما سقف اجرای ابزارها در این مرحله رسید. نتیجه‌های انجام‌شده حفظ شده‌اند و می‌توان با گفتن «ادامه بده» کار را از وضعیت فعلی ادامه داد.$suffix"
        }
    }
    ...
    private suspend fun executeTool(
        ...
        name == "working_memory" -> workingMemory.tool(
            conversationId,
            JSONObject(arguments),
        ).toString()
        name == "product_categories_list" ||
            (name == "products_list" && JSONObject(arguments).optLong("categoryId", 0L) > 0L) ->
    ...
    private fun batchLabel(name: String, arguments: String): String =
        "${toolDisplayName(name)} · ${formatToolArguments(arguments).replace("\n", " · ").take(180)}"

    private fun tokenForBatch(items: List<PendingBatchItem>) =
        sha256(items.joinToString("|") { "${it.callId}:${it.name}:${it.arguments}" }).take(32)

    private fun writeFailureMessage(result: JSONObject) =
        result.optString("error").ifBlank { "عملیات تغییر انجام نشد یا قابل تأیید نیست." }

    private fun summarize(result: String) =
        result.replace("\n", " ").trim().let {
            if (it.length > 140) it.take(137) + "..." else it
        }

    private fun tokenFor(name: String, arguments: String) =
        sha256("$name:$arguments").take(32)
