...
fun SettingsScreen(storeName:String,storeId:StoreId,onBack:()->Unit,onDisconnect:()->Unit,dependencies:V1PresentationDependencies){
 val context=LocalContext.current;val clipboard=LocalClipboardManager.current;val scope=rememberCoroutineScope();val transfer=remember(dependencies){ProductTransferService(dependencies,context.contentResolver)}
 var logs by remember{mutableStateOf(DebugLogStore.read(context))};var showSalesDebug by remember{mutableStateOf(true)};var busy by remember{mutableStateOf(false)};var progress by remember{mutableStateOf("")};var resultText by remember{mutableStateOf<String?>(null)}
 ...
 val importLauncher=rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()){uri->if(uri!=null){context.contentResolver.takePersistableUriPermission(uri,Intent.FLAG_GRANT_READ_URI_PERMISSION);busy=true;scope.launch{val r=transfer.import(storeId,uri){progress=it.phase};busy=false;resultText="ایمپورت تمام شد. ایجاد: ${r.created} · بروزرسانی: ${r.updated} · ناموفق: ${r.failed} · تصاویر: ${r.imagesUploaded} · Variation ایجاد: ${r.variationsCreated} · Variation بروزرسانی: ${r.variationsUpdated}${if(r.errors.isNotEmpty())"\n\n${r.errors.take(5).joinToString("\n")}" else ""}"}}}
