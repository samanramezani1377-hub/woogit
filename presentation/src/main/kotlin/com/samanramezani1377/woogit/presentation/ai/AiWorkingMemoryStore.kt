...
internal class AiWorkingMemoryStore(
    context: Context,
    private val storeKey: String,
) {
    private val prefs = context.applicationContext.getSharedPreferences(
        "woogit_ai_working_memory",
        Context.MODE_PRIVATE,
    )
    private val prefix = "working_${storeKey}_"

    @Synchronized
    fun read(conversationId: String): JSONObject? =
        readRaw(conversationId)?.let(::promptSnapshot)
    ...
    fun ensure(conversationId: String, task: String): JSONObject {
        ...
        return newState(conversationId, task).also {
            write(conversationId, it)
        }
    }
        ...
        if (previous != null) {
            ...
            history.put(
                JSONObject().apply {
                    ...
                    put(
                        "progress",
                        previous.optJSONObject("progress") ?: JSONObject(),
                    )
                    put(
                        "checkpoint",
                        previous.optJSONObject("checkpoint") ?: JSONObject(),
                    )
            ...
            state.put(
                "batches",
                tail(previous.optJSONArray("batches"), BATCH_HISTORY_LIMIT),
            )
        }
    ...
    fun resume(conversationId: String): JSONObject {
        val state = readRaw(conversationId)
            ?: return JSONObject().put("status", "empty")
    ...
    fun beginInFlightOperation(
        ...
        val state = readRaw(conversationId)
            ?: newState(
                conversationId,
                operation.optString("task", "Agent task"),
            )
        val fingerprint = operation.optString("fingerprint")
            .ifBlank {
                fingerprintOf(
                    operation.optString("tool"),
                    operation.optString("arguments"),
                )
            }
        ...
        state
            ...
            .put("lastOperation", operation.optString("tool"))
    ...
    fun createBatch(
        ...
        val batch = JSONObject()
            ...
            .put("items", JSONArray(items.toString()))
    ...
    fun updateBatchItem(
        ...
        val batches = state.optJSONArray("batches")
            ?: return promptSnapshot(state)

        for (i in 0 until batches.length()) {
            val batch = batches.optJSONObject(i) ?: continue
            if (batch.optString("batchId") != batchId) continue
            ...
            for (j in 0 until items.length()) {
                ...
                if (result.isNotBlank()) {
                    item.put("result", result.take(MAX_RESULT_LENGTH))
                }
                break
            }
            ...
            if (counts.optInt("pending", 0) == 0) {
                batch.put(
                    "status",
                    if (counts.optInt("failed", 0) > 0) {
    ...
    fun update(conversationId: String, data: JSONObject): JSONObject {
        val state = readRaw(conversationId)
            ?: newState(conversationId, data.optString("task", "Agent task"))

        data.optString("task")
            .takeIf { it.isNotBlank() }
            ?.let { state.put("task", it.take(MAX_TASK_LENGTH)) }
        data.optString("status")
            .takeIf { it.isNotBlank() }
            ?.let { state.put("status", it) }
        data.optString("summary")
            .takeIf { it.isNotBlank() }
            ?.let { state.put("summary", it.take(MAX_SUMMARY_LENGTH)) }
        data.optString("lastOperation")
            .takeIf { it.isNotBlank() }
            ?.let { state.put("lastOperation", it) }
        data.optString("nextOperation")
            .takeIf { it.isNotBlank() }
            ?.let { state.put("nextOperation", it) }

        data.optJSONObject("progress")?.let {
            state.put(
                "progress",
                mergeObject(state.optJSONObject("progress") ?: JSONObject(), it),
            )
        }
        data.optJSONObject("checkpoint")?.let {
            state.put("checkpoint", JSONObject(it.toString()))
        }
    ...
    fun recordOperation(
        ...
        val state = readRaw(conversationId)
            ?: newState(conversationId, operation.optString("task", "Agent task"))
        val operations = state.optJSONArray("operations") ?: JSONArray()
        val normalized = JSONObject(operation.toString())
        val fingerprint = normalized.optString("fingerprint")
            .ifBlank {
                fingerprintOf(
                    normalized.optString("tool"),
    ...
    fun checkpoint(conversationId: String, data: JSONObject): JSONObject {
        ...
        state.put(
            "checkpoint",
            JSONObject((data.optJSONObject("checkpoint") ?: data).toString()),
        )
        data.optJSONObject("progress")?.let {
            state.put(
                "progress",
    ...
    fun complete(conversationId: String, summary: String): JSONObject? {
        ...
        state
            ...
            .put("summary", summary.take(MAX_SUMMARY_LENGTH))
            .put("updatedAt", System.currentTimeMillis())
    ...
    @Synchronized
    fun clear(conversationId: String): Boolean =
        prefs.edit().remove(key(conversationId)).commit()

    fun tool(conversationId: String, arguments: JSONObject): JSONObject =
        runCatching {
            when (arguments.optString("operation").trim().lowercase()) {
                "read" -> read(conversationId)
                    ?: JSONObject().put("status", "empty")
                "update" -> update(
                    conversationId,
                    arguments.optJSONObject("data") ?: JSONObject(),
                )
                ...
                "compact" -> compact(conversationId)
                    ?: JSONObject().put("status", "empty")
                "complete" -> complete(
                    conversationId,
                    arguments.optString("summary"),
                ) ?: JSONObject().put("status", "empty")
                "clear" -> JSONObject().put("cleared", clear(conversationId))
                else -> JSONObject().put(
                    "error",
                    "unknown working memory operation",
                )
            }
        }.getOrElse {
            JSONObject().put(
                "error",
                it.message ?: "working memory operation failed",
            )
        }
    ...
    private fun readRaw(conversationId: String): JSONObject? =
        runCatching {
    ...
    private fun promptSnapshot(state: JSONObject): JSONObject =
        JSONObject().apply {
            put("version", state.optInt("version", 4))
            put("executionId", state.optString("executionId"))
            put("conversationId", state.optString("conversationId"))
            ...
            put("lastOperation", state.optString("lastOperation"))
            put("nextOperation", state.optString("nextOperation"))
            put("summary", state.optString("summary"))
            put(
                "operations",
                tail(state.optJSONArray("operations"), PROMPT_OPERATION_LIMIT),
            )
            put("errors", tail(state.optJSONArray("errors"), PROMPT_ERROR_LIMIT))
            put("batches", tail(state.optJSONArray("batches"), BATCH_HISTORY_LIMIT))
        }
    ...
    private fun countBatchStatuses(items: JSONArray): JSONObject {
        ...
        for (i in 0 until items.length()) {
            when (items.optJSONObject(i)?.optString("status")) {
                "APPROVED", "NOT_STARTED", "IN_FLIGHT" -> counts.put(
                    "pending",
                    counts.optInt("pending") + 1,
                )
                "REJECTED_BY_USER" -> counts.put(
                    "rejected",
                    counts.optInt("rejected") + 1,
                )
                "VERIFIED" -> counts.put(
                    "verified",
                    counts.optInt("verified") + 1,
                )
                "FAILED" -> counts.put(
                    "failed",
                    counts.optInt("failed") + 1,
                )
                else -> counts.put(
                    "pending",
                    counts.optInt("pending") + 1,
    ...
    private fun tail(source: JSONArray?, limit: Int): JSONArray {
        ...
        for (i in maxOf(0, source.length() - limit) until source.length()) {
    ...
    private fun compactInPlace(state: JSONObject) {
        state.put(
            "operations",
            tail(state.optJSONArray("operations"), COMPACT_OPERATION_LIMIT),
        )
        state.put(
            "errors",
            tail(state.optJSONArray("errors"), COMPACT_ERROR_LIMIT),
        )
        state.put(
            "batches",
            tail(state.optJSONArray("batches"), BATCH_HISTORY_LIMIT),
        )
    ...
    private fun fingerprintOf(tool: String, arguments: String): String =
        "$tool|$arguments".hashCode().toString()

    private fun write(conversationId: String, state: JSONObject) {
        prefs.edit()
            .putString(key(conversationId), state.toString())
            .commit()
    }
