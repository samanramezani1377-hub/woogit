...
internal fun validateTransferProduct(x:TransferProduct):String?=when{ x.name.isBlank()->"محصول با شناسه ${x.id} فاقد نام است و وارد نشد.";x.type.isBlank()->"محصول «${x.name}» فاقد نوع (type) است و وارد نشد.";x.status.isBlank()->"محصول «${x.name}» فاقد وضعیت (status) است و وارد نشد.";else->null }
