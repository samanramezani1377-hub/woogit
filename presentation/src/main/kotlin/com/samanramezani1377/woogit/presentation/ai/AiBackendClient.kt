...
internal class AiBackendClient(
    ...
    fun agent(messages: List<Pair<String, String>>, confirmationToken: String? = null): AgentReply {
        ...
        if (confirmationToken != null) {
            ...
            val output = execute(action.name, action.arguments)
            working.put(assistantToolCall(action.callId, action.name, action.arguments))
            working.put(JSONObject().put("role", "tool").put("tool_call_id", action.callId).put("content", output))
        }

        repeat(MAX_STEPS) {
            val response = request(working)
            val message = response.optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("message")
                ?: throw IllegalStateException("DeepSeek پاسخ معتبری برنگرداند.")
    ...
    private fun request(messages: JSONArray): JSONObject {
        ...
        val body = JSONObject().put("model", "deepseek-v4-flash").put("messages", messages)
            .put("thinking", JSONObject().put("type", "disabled"))
            .put("stream", false).put("tools", toolDefinitions()).put("tool_choice", "auto")
    ...
    private fun toolDefinitions() = JSONArray().apply {
        ...
        put(tool("products_create", "ایجاد محصول از مسیر رسمی WooGit؛ نیازمند تأیید کاربر.", JSONObject().apply {
        ...
        put(tool("orders_update_status", "تغییر وضعیت سفارش از طریق WooGit؛ نیازمند تأیید کاربر.", JSONObject().apply {
    ...
    private fun execute(name: String, raw: String): String {
    ...
    private fun updateProduct(a: JSONObject): CoreResult<*> {
        val id = EntityId(a.getLong("id").toString())
        return when (val current = dependencies.getProduct(storeId, id)) {
            is CoreResult.Success -> {
                val p = current.value; val patch = a.getJSONObject("patch")
                val pricing = p.pricing.copy(
    ...
    private fun updateOrderStatus(a: JSONObject): CoreResult<*> {
        val id = EntityId(a.getLong("id").toString())
    ...
    private fun stringify(v: Any?): Any = when (v) {
        ...
        is Order -> JSONObject().put("id", v.id.value).put("number", v.number).put("status", v.status.name).put("total", v.total ?: JSONObject.NULL).put("currency", v.currency ?: JSONObject.NULL).put("customer", v.customer?.name ?: JSONObject.NULL)
        is List<*> -> JSONArray().apply { v.forEach { put(stringify(it)) } }
        else -> v.toString()
    }

    private fun productJson(p: Product) = JSONObject().apply {
        put("id", p.id.value); put("name", p.name); put("sku", p.sku ?: JSONObject.NULL); put("status", p.status.name); put("type", p.type.name)
        ...
        put("categories", JSONArray().apply { p.categories.forEach { put(JSONObject().put("id", it.id.value).put("name", it.name)) } })
    }
