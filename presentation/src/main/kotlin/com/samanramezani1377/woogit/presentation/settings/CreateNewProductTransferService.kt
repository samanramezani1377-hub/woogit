...
@Serializable private data class CreateProduct(
    val id: String,
    val name: String,
    val sku: String? = null,
    ...
    val categories: List<CreateIdName> = emptyList(),
    val attributes: List<CreateAttribute> = emptyList(),
    val images: List<CreateImage> = emptyList(),
    val variations: List<CreateVariation> = emptyList(),
)
@Serializable private data class CreateVariation(
    val id: String,
    val sku: String? = null,
    val regular: String? = null,
    val sale: String? = null,
    ...
    val stockStatus: String? = null,
    val manageStock: Boolean,
    val attributes: List<CreateVariationAttribute> = emptyList(),
    val image: CreateImage? = null,
)
...
class CreateNewProductTransferService(private val d: V1PresentationDependencies, private val resolver: ContentResolver) {
            ...
            pack.products.forEachIndexed { index, x ->
                ...
                try {
                    ...
                    x.variations.forEach { v ->
                        ...
                        val variation = v.toDomain(saved.id, image)
                        when (val r = d.createVariation(storeId, variation)) {
                    ...
                    failed++
                    errors += "${x.name}: ${t.message ?: "خطای نامشخص"}"
                }
            }
            RobustProductTransferResult(created = created, updated = 0, failed = failed, imagesUploaded = imagesUploaded, variationsCreated = variationsCreated, variationsUpdated = 0, errors = errors.distinct().take(50))
        } catch (t: Throwable) {
    ...
    private suspend fun readPackage(uri: Uri, onProgress: (ProductTransferProgress) -> Unit): CreateReadPackage = withContext(Dispatchers.IO) {
        ...
        var products: List<CreateProduct>? = null
        ...
        resolver.openInputStream(uri)?.use { input -> ZipInputStream(input).use { zip ->
            while (true) {
                ...
                when (name) {
                    "manifest.json" -> manifest = createNewJson.decodeFromString(bytes.toString(Charsets.UTF_8))
                    "products.json" -> { val p = createNewJson.decodeFromString<CreatePackage>(bytes.toString(Charsets.UTF_8)); manifest = manifest ?: p.manifest; products = p.products }
                    else -> if (name.startsWith("media/")) media[name] = bytes
                }
            }
        ...
        require(m.images == media.size) { "برخی تصاویر فایل قابل وارد کردن نیستند." }
    ...
    private fun readLimited(input: ZipInputStream, maxBytes: Long, onBytes: (Long) -> Unit): ByteArray {
        ...
        while (true) {
            ...
            if (read <= 0) break
            total += read
            require(total <= maxBytes) { "یکی از فایل‌های داخل بسته بیش از حد بزرگ است." }
            onBytes(read.toLong())
    ...
    private suspend fun allCategories(storeId: StoreId): List<IdName> {
        val result = mutableListOf<IdName>()
        var page = 1
        while (true) {
            val batch = when (val r = d.getProductCategories(storeId, page, 100, null)) { is CoreResult.Success -> r.value; is CoreResult.Failure -> break }
...
private fun CreateProduct.toDomain(images: List<ProductImage>, categories: List<IdName>, attributes: List<Attribute>) = Product(
    id = EntityId(id), name = name, sku = sku, description = description, shortDescription = shortDescription,
    ...
    images = images, categories = categories, attributes = attributes, modifiedAt = null,
)

private fun CreateVariation.toDomain(productId: EntityId, image: ProductImage?) = Variation(
    id = EntityId(id), productId = productId, attributes = attributes.map { VariationAttribute(it.name, it.option) },
    pricing = Pricing(regular, sale, onSale),
    stock = if (quantity != null || stockStatus != null || manageStock) Stock(quantity, runCatching { StockStatus.valueOf(stockStatus ?: StockStatus.IN_STOCK.name) }.getOrDefault(StockStatus.IN_STOCK), manageStock) else null,
    sku = sku, image = image, modifiedAt = null,
)
