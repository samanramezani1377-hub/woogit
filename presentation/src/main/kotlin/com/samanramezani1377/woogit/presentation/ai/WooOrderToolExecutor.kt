...
/**
 * Owns order tools and the model-facing order JSON contract.
 * The main tool router delegates here; provider/context behavior is unchanged.
 */
internal class WooOrderToolExecutor(
    ...
    private suspend fun list(a: JSONObject): String {
    ...
    private suspend fun updateStatus(a: JSONObject): String {
        ...
        val requested = runCatching {
            OrderStatus.valueOf(a.getString("status").trim().uppercase())
        ...
        return when (val current = dependencies.getOrder(storeId, id)) {
            ...
            is CoreResult.Success -> when (
                val result = dependencies.updateOrder(storeId, id, current.value.copy(status = requested))
            ) {
                is CoreResult.Failure -> failure(result.error.toString())
                is CoreResult.Success -> when (val reread = dependencies.getOrder(storeId, id)) {
                    ...
                    is CoreResult.Success -> if (reread.value.status == requested) {
                        JSONObject()
                            .put("ok", true)
                            .put("verified", true)
                            .put("operation", "orders_update_status")
                            .put("data", JSONObject()
                                .put("orderId", id.value)
                                .put("status", requested.name)
                                .put("statusLabel", statusLabel(requested))
                            )
                            .toString()
                    } else {
                        failure("تغییر وضعیت سفارش تأیید نشد؛ وضعیت نهایی ${reread.value.status.name} است.")
                    }
                }
            }
        }
    ...
    private fun orderJson(order: Order): JSONObject = JSONObject()
        ...
        .put("statusLabel", statusLabel(order.status))
        .put("total", order.total ?: JSONObject.NULL)
        .put("currency", order.currency ?: JSONObject.NULL)
        .put("customer", order.customer?.let(::customerJson) ?: JSONObject.NULL)
        .put("billing", order.billing?.let(::addressJson) ?: JSONObject.NULL)
        .put("shipping", order.shipping?.let(::addressJson) ?: JSONObject.NULL)
    ...
    private fun customerJson(value: Customer) = JSONObject()
        .put("id", value.id?.value ?: JSONObject.NULL)
        .put("name", value.name)
        .put("email", value.email ?: JSONObject.NULL)

    private fun addressJson(value: Address) = JSONObject()
        .put("firstName", value.firstName ?: JSONObject.NULL)
        .put("lastName", value.lastName ?: JSONObject.NULL)
        .put("company", value.company ?: JSONObject.NULL)
        .put("address1", value.address1 ?: JSONObject.NULL)
        .put("address2", value.address2 ?: JSONObject.NULL)
        .put("city", value.city ?: JSONObject.NULL)
        .put("state", value.state ?: JSONObject.NULL)
        .put("postcode", value.postcode ?: JSONObject.NULL)
        .put("country", value.country ?: JSONObject.NULL)
        .put("phone", value.phone ?: JSONObject.NULL)

    private fun paymentJson(value: Payment) = JSONObject()
        .put("methodId", value.methodId ?: JSONObject.NULL)
        .put("methodTitle", value.methodTitle ?: JSONObject.NULL)
        .put("transactionId", value.transactionId ?: JSONObject.NULL)
        .put("paid", value.paid)

    private fun shippingJson(value: ShippingLine) = JSONObject()
        .put("methodId", value.methodId ?: JSONObject.NULL)
        .put("methodTitle", value.methodTitle ?: JSONObject.NULL)
        .put("total", value.total ?: JSONObject.NULL)

    private fun discountJson(value: Discount) = JSONObject()
        .put("code", value.code)
        .put("total", value.total)

    private fun noteJson(value: OrderNote) = JSONObject()
        .put("id", value.id?.value ?: JSONObject.NULL)
        .put("content", value.content)
        .put("customerNote", value.customerNote)

    private fun itemJson(value: OrderItem) = JSONObject()
        .put("id", value.id.value)
        .put("productId", value.productId?.value ?: JSONObject.NULL)
        .put("variationId", value.variationId?.value ?: JSONObject.NULL)
        .put("name", value.name)
        .put("quantity", value.quantity)
        .put("subtotal", value.subtotal)
        .put("total", value.total)
