...
internal fun AiComposer(
    ...
    Row(
        ...
        IconButton(onClick = onPickImage, enabled = enabled) { Text("+", color = GlassTokens.accent, fontWeight = FontWeight.Bold) }
        GlassTextField(
            value = input,
        ...
        if (isGenerating) {
            IconButton(onClick = onStop, modifier = Modifier.size(52.dp)) {
                Box(Modifier.size(44.dp).clip(RoundedCornerShape(3.dp)).background(GlassTokens.urgent), contentAlignment = Alignment.Center) {
                    Box(Modifier.size(16.dp).clip(RoundedCornerShape(3.dp)).background(Color.White))
                }
            }
        } else {
            IconButton(onClick = onSend, enabled = enabled, modifier = Modifier.size(52.dp)) {
