...
internal class CloudflareProvider(context: Context) : AiProvider {
    ...
    private fun body(messages: JSONArray, tools: JSONArray, stream: Boolean, model: String) = JSONObject()
        ...
        .put("parallel_tool_calls", false).put("max_completion_tokens", MAX_COMPLETION_TOKENS)

    private fun connection(account: String, token: String) = (URL("https://api.cloudflare.com/client/v4/accounts/$account/ai/v1/chat/completions").openConnection() as HttpURLConnection).apply {
    ...
    private fun httpError(connection: HttpURLConnection, status: Int): IllegalStateException = httpErrorText(status, connection.errorStream?.bufferedReader()?.use { it.readText() }.orEmpty())
    private fun httpErrorText(status: Int, text: String): IllegalStateException {
        val message = runCatching { val root = JSONObject(text); root.optJSONObject("error")?.optString("message")?.takeIf { it.isNotBlank() } ?: root.optJSONArray("errors")?.optJSONObject(0)?.optString("message") }.getOrNull().orEmpty()
