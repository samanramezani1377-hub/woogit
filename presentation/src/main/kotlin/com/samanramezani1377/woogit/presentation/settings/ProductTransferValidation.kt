...
internal fun validateTransferPackage(resolver:ContentResolver,uri:Uri,json:Json):TransferValidatedPackage{
    ...
    resolver.openInputStream(uri)?.use{input->ZipInputStream(input).use{zip->while(true){val entry=zip.nextEntry?:break;if(entry.isDirectory)continue;val name=entry.name;require(name.isNotBlank()&&!name.startsWith("/")&&!name.startsWith("\\")&&!name.split('/','\\').any{it==".."}){"مسیر نامعتبر داخل فایل WooGit."};require(seenEntries.add(name)){"ورودی تکراری داخل فایل WooGit: $name"};require(name=="manifest.json"||name=="products.json"||name.startsWith("media/")){"ورودی ناشناخته داخل فایل WooGit: $name"};val b=readTransferEntry(zip,if(name=="products.json")MAX_PRODUCT_JSON_BYTES else MAX_ENTRY_BYTES);total+=b.size;require(total<=MAX_PACKAGE_BYTES){"حجم فایل WooGit بیش از حد مجاز است."};when(name){"manifest.json"->manifest=json.decodeFromString(b.toString(Charsets.UTF_8));"products.json"->{val pack=json.decodeFromString<ProductTransferPackage>(b.toString(Charsets.UTF_8));manifest=manifest?:pack.manifest;products=pack.products;globals=pack.globalAttributes};else->mediaNames+=name}}}}?:error("فایل قابل خواندن نیست.")
    val m=requireNotNull(manifest){"manifest.json در فایل وجود ندارد."};val p=requireNotNull(products){"products.json در فایل وجود ندارد."};require(m.format==FORMAT){"فرمت فایل WooGit معتبر نیست."};require(m.version in 1..VERSION){"نسخه فایل پشتیبانی نمی‌شود."};require(m.products==p.size){"تعداد محصولات فایل با manifest سازگار نیست."};require(p.size<=MAX_PRODUCTS){"تعداد محصولات فایل بیش از حد مجاز است."};require(m.images==mediaNames.size){"تعداد تصاویر فایل با manifest سازگار نیست."}
