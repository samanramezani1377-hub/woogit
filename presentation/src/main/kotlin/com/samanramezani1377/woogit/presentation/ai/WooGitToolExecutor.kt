...
internal class WooGitToolExecutor(
    ...
    private suspend fun createProduct(a: JSONObject): String {
        val requested = Product(
            ...
            ProductType.SIMPLE,
            Pricing(a.optString("regularPrice").takeIf { it.isNotBlank() }, null, false),
        ...
        return when (val created = dependencies.createProduct(storeId, requested)) {
            ...
            is CoreResult.Success -> {
                val product = created.value as? Product
                if (product == null) return failure("ایجاد محصول انجام شد اما محصول ایجادشده قابل تأیید نبود.")
                verifyProduct(product, a, operation = "ایجاد محصول")
            }
        }
    }
        ...
        return when (val current = dependencies.getProduct(storeId, id)) {
            ...
            is CoreResult.Success -> {
                ...
                when (val result = dependencies.updateProduct(storeId, id, updated)) {
                    ...
                    is CoreResult.Success -> {
                        val refreshed = when (val reread = dependencies.getProduct(storeId, id)) {
                            is CoreResult.Failure -> return failure("تغییر محصول ارسال شد، اما وضعیت نهایی قابل تأیید نیست: ${reread.error}")
                            is CoreResult.Success -> reread.value
                        }
                        verifyProduct(refreshed, patch, operation = "ویرایش محصول")
                    }
                }
            }
        }
    ...
    private suspend fun updateOrderStatus(a: JSONObject): String {
        val id = EntityId(a.getLong("id").toString())
        val requested = a.getString("status")
        return when (val current = dependencies.getOrder(storeId, id)) {
            ...
            is CoreResult.Success -> when (val result = dependencies.updateOrder(storeId, id, current.value.copy(status = OrderStatus.valueOf(requested)))) {
                ...
                is CoreResult.Success -> when (val reread = dependencies.getOrder(storeId, id)) {
                    ...
                    is CoreResult.Success -> if (reread.value.status == OrderStatus.valueOf(requested)) {
                        JSONObject().put("ok", true).put("verified", true).put("operation", "orders_update_status").put("message", "وضعیت سفارش با موفقیت تغییر و تأیید شد.").toString()
                    } else {
    ...
    private fun verifyProduct(product: Product, patch: JSONObject, operation: String): String {
        ...
        return if (mismatches.isEmpty()) {
            JSONObject().put("ok", true).put("verified", true).put("operation", operation).put("data", productJson(product)).toString()
        } else {
            failure("$operation ارسال شد، اما مقدار نهایی با درخواست یکسان نیست: ${mismatches.joinToString(", ")}")
        }
    }
