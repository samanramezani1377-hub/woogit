...
internal class WooGitToolExecutor(
                ...
                when (val saved = dependencies.updateProduct(storeId, id, product.copy(images = reordered))) {
                    ...
                    is CoreResult.Success -> when (val reread = dependencies.getProduct(storeId, id)) {
                        ...
                        is CoreResult.Success -> {
                            ...
                            if (first?.id?.value == selected.id?.value || (selected.id == null && first?.src == selected.src)) JSONObject().put("ok", true).put("verified", true).put("operation", "products_image_set_primary").toString()
    ...
    private suspend fun removeProductImage(a: JSONObject): String {
        ...
        val imageId = a.optString("imageId").trim().takeIf { it.isNotBlank() }
        return when (val current = dependencies.getProduct(storeId, id)) {
            is CoreResult.Failure -> failure(current.error.toString())
            is CoreResult.Success -> {
                val product = current.value
                ...
                when (val saved = dependencies.updateProduct(storeId, id, product.copy(images = remaining))) {
                    ...
                    is CoreResult.Success -> when (val reread = dependencies.getProduct(storeId, id)) {
                        ...
                        is CoreResult.Success -> {
                            val stillPresent = reread.value.images.any(matches)
    ...
    private fun productSummaryJson(p: Product) = JSONObject()
        ...
        .put("permalink", p.permalink ?: JSONObject.NULL)
        .put("variationCount", p.variationCount ?: JSONObject.NULL)
        ...
        .put("pricing", JSONObject()
            .put("regular", p.pricing.regular ?: JSONObject.NULL)
            .put("sale", p.pricing.sale ?: JSONObject.NULL)
            .put("onSale", p.pricing.onSale)
            .put("currency", p.currency ?: JSONObject.NULL)
            .put("currencySymbol", p.currencySymbol ?: JSONObject.NULL)
            .put("currencyPosition", p.currencyPosition ?: JSONObject.NULL)
            .put("thousandSeparator", p.thousandSeparator ?: JSONObject.NULL)
            .put("decimalSeparator", p.decimalSeparator ?: JSONObject.NULL)
            .put("numberOfDecimals", p.numberOfDecimals ?: JSONObject.NULL))
        .put("commerce", JSONObject()
            .put("currency", p.currency ?: JSONObject.NULL)
            .put("currencySymbol", p.currencySymbol ?: JSONObject.NULL)
            .put("currencyPosition", p.currencyPosition ?: JSONObject.NULL)
            .put("thousandSeparator", p.thousandSeparator ?: JSONObject.NULL)
            .put("decimalSeparator", p.decimalSeparator ?: JSONObject.NULL)
            .put("numberOfDecimals", p.numberOfDecimals ?: JSONObject.NULL)
            .put("weightUnit", p.weightUnit ?: JSONObject.NULL)
            .put("dimensionUnit", p.dimensionUnit ?: JSONObject.NULL))
