...
private fun ProductEditorUiState.Editing.toProduct(original:Product?):Product{
    ...
    val cats=categories.split(',').map{it.trim()}.filter{it.isNotBlank()}.mapIndexed{index,value->original?.categories?.getOrNull(index)?.let{id->IdName(id.id,value)}?:IdName(EntityId("category-$index"),value)}
    val attrs=attributes.split('|').mapNotNull{raw->val parts=raw.split(':',limit=2);if(parts.size!=2||parts[0].isBlank())null else{val old=original?.attributes?.firstOrNull{it.name==parts[0].trim()};Attribute(old?.id,parts[0].trim(),old?.visible?:true,old?.variation?:true,parts[1].split(',').map{it.trim()}.filter{it.isNotBlank()})}}
    return Product(EntityId(productId ?: "new"),name.trim(),sku.trim().ifBlank{null},description.ifBlank{null},shortDescription.ifBlank{null},original?.status?:ProductStatus.DRAFT,original?.type?:ProductType.SIMPLE,Pricing(price.trim().ifBlank{null},salePrice.trim().ifBlank{null},salePrice.isNotBlank()),stock.toDoubleOrNull()?.let{Stock(it,if(it>0)StockStatus.IN_STOCK else StockStatus.OUT_OF_STOCK,original?.stock?.manageStock?:true)},if(image!=null)listOf(image) else original?.images.orEmpty(),cats,attrs,original?.modifiedAt)
}
