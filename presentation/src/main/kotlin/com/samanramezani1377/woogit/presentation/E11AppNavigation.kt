...
internal fun E11AppNavigation(dependencies: V1PresentationDependencies, initialOrderId: String?) {
    ...
    NavHost(navController, startDestination) {
        ...
        composable(E11Routes.ORDERS) { val s = activeStore?.let(::StoreId); if (s != null) { val vm = viewModel<OrdersViewModel>(factory = vmFactory { OrdersViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s) { vm.load(s) }; OrdersScreen(mapOrdersState(st), { id -> navController.navigate(E11Routes.order(id)) }, { vm.load(s) }, { vm.nextPage(s) }, { vm.load(s, it, true) }) } }
        composable(E11Routes.ORDER_DETAIL, listOf(navArgument(E11Routes.ORDER_ID) { type = NavType.StringType })) { e ->
            ...
            if (s != null && id != null) {
                val vm = viewModel<OrderDetailViewModel>(factory = vmFactory { OrderDetailViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s, id) { vm.load(s, EntityId(id)) }
                val current = (st as? FeatureUiState.Success)?.value
                OrderDetailScreen(mapOrderDetailState(st), { vm.load(s, EntityId(id)) }, { navController.popBackStack() }, { status -> current?.let { vm.update(s, it.copy(status = status)) } })
            }
        }
        composable(E11Routes.PRODUCTS) { val s = activeStore?.let(::StoreId); if (s != null) { val vm = viewModel<ProductsViewModel>(factory = vmFactory { ProductsViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s) { vm.load(s) }; ProductsScreen(st, { id -> navController.navigate(E11Routes.product(id)) }, { vm.load(s) }, { vm.nextPage(s) }, { vm.load(s, it, true) }, { navController.navigate(E11Routes.PRODUCT_NEW) }) } }
