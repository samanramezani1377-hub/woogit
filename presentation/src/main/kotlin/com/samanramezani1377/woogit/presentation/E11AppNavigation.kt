    ...
    NavHost(navController, startDestination) {
        ...
        composable(E11Routes.DASHBOARD) {
            ...
            if (store != null) {
                ...
                DashboardScreen(
                    ...
                    state.pendingCount,
                    recent?.number,
                    recent?.customer?.name.orEmpty(),
                    formatMoney(recent?.total),
                    recent?.status,
                    { recent?.number?.let { navController.navigate(E11Routes.order(it)) } },
        ...
        composable(E11Routes.ORDERS) {
            ...
            if (store != null) {
                ...
                val vm = viewModel<OrdersViewModel>(
                    ...
                    factory = vmFactory { OrdersViewModel(dependencies) },
                )
        ...
        composable(E11Routes.ORDER_DETAIL, listOf(navArgument(E11Routes.ORDER_ID) { type = NavType.StringType })) { entry ->
            ...
            if (store != null && id != null) {
                val storeId = StoreId(store)
                val vm = viewModel<OrderDetailViewModel>(
                    key = "order-detail-${storeId.value}-$id",
                    factory = vmFactory { OrderDetailViewModel(dependencies) },
                )
                val state by vm.state.collectAsState()
        ...
        composable(E11Routes.PRODUCTS) {
            ...
            if (store != null) {
                ...
                val vm = viewModel<ProductsViewModel>(
                    ...
                    factory = vmFactory { ProductsViewModel(dependencies) },
                )
                val state by vm.state.collectAsState()
                LaunchedEffect(storeId) { vm.load(storeId) }
        ...
        composable(E11Routes.PRODUCT_DETAIL, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { entry ->
            ...
            if (store != null && id != null) {
                val storeId = StoreId(store)
                val vm = viewModel<ProductDetailViewModel>(
                    key = "product-detail-${storeId.value}-$id",
                    factory = vmFactory { ProductDetailViewModel(dependencies) },
                )
                val state by vm.state.collectAsState()
                LaunchedEffect(storeId, id) { vm.load(storeId, EntityId(id)) }
        ...
        composable(E11Routes.VARIATIONS, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { entry ->
            ...
            if (store != null && id != null) {
                val storeId = StoreId(store)
                val vm = viewModel<VariationsViewModel>(
                    key = "variations-${storeId.value}-$id",
                    factory = vmFactory { VariationsViewModel(dependencies) },
                )
                val state by vm.state.collectAsState()
                LaunchedEffect(storeId, id) { vm.load(storeId, EntityId(id)) }
...
private fun mapOrderDetailState(state: FeatureUiState<Order>): OrderDetailUiState = when (state) {
    ...
    is FeatureUiState.Conflict -> OrderDetailUiState.Error("تعارض در داده‌های سفارش.")
...
private fun mapSyncState(state: FeatureUiState<SyncMetadata>): SyncUiState = when (state) {
    ...
    is FeatureUiState.Conflict -> SyncUiState.Error("تعارض در داده‌های همگام‌سازی وجود دارد.")
}
