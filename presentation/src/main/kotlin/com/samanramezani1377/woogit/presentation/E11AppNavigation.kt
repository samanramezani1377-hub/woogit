    ...
    NavHost(navController, startDestination) {
        ...
        composable(E11Routes.DASHBOARD) {
            ...
            if (store == null) {
                ...
                DashboardScreen(s.value, st.connectionState == ConnectionState.CONNECTED, st.orders.size.toString(), st.products.size.toString(), st.orders.sumOf { it.total?.toDoubleOrNull() ?: 0.0 }.toString(), st.orders.count { it.status == OrderStatus.PENDING }.toString(), r?.number, r?.customer?.name.orEmpty(), formatMoney(r?.total), r?.status, { r?.number?.let { navController.navigate(E11Routes.order(it)) } }, { navController.navigate(E11Routes.ORDERS) }, { navController.navigate(E11Routes.PRODUCTS) }, { navController.navigate(E11Routes.SETTINGS) }, { navController.navigate(E11Routes.SYNC) }, { navController.navigate(E11Routes.CONFLICTS) }, DashboardDestination.DASHBOARD, { d -> when (d) { DashboardDestination.DASHBOARD -> Unit; DashboardDestination.ORDERS -> navController.navigate(E11Routes.ORDERS); DashboardDestination.PRODUCTS -> navController.navigate(E11Routes.PRODUCTS); DashboardDestination.SETTINGS -> navController.navigate(E11Routes.SETTINGS) } })
            }
        }
        ...
        composable("products/{productId}/edit", listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { e -> val store = activeStore; val id = e.arguments?.getString(E11Routes.PRODUCT_ID); if (store != null && id != null) ProductEditorRoute(dependencies, StoreId(store), id, { navController.popBackStack() }, { navController.popBackStack() }) }
