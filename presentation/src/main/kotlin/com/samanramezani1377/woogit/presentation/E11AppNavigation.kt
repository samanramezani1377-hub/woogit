...
import com.samanramezani1377.woogit.presentation.ai.ChatGptAgentWebViewScreen
...
internal fun E11AppNavigation(
    ...
    val accountSetupViewModel = viewModel<AccountSetupViewModel>(factory = vmFactory { AccountSetupViewModel(accountSetupGateway) })
    val accountSetupState by accountSetupViewModel.state.collectAsState()

    LaunchedEffect(dependencies.initialStoreId) {
        dependencies.initialStoreId?.let { persistedStore -> if (activeStore != persistedStore) activeStore = persistedStore }
    }
    ...
    NavHost(navController, startDestination) {
            ...
            if (store != null) {
                ...
                LiquidGlassEnvironment {
                    DashboardScreen(
                        ...
                        state.processingCount, state.orders.firstOrNull()?.number, state.orders.firstOrNull()?.customer?.name.orEmpty(),
                        formatMoney(state.orders.firstOrNull()?.total), state.orders.firstOrNull()?.status,
                        { state.orders.firstOrNull()?.number?.let { navController.navigate(E11Routes.order(it)) } },
                        { navController.navigate(E11Routes.ORDERS) }, { navController.navigate(E11Routes.PRODUCTS) }, { navController.navigate(E11Routes.SETTINGS) },
        ...
        composable(E11Routes.ORDERS) {
            ...
            if (store != null) {
                val storeId = StoreId(store)
                val vm = viewModel<OrdersViewModel>(key = "orders-${storeId.value}", factory = vmFactory { OrdersViewModel(dependencies) })
        ...
        composable(E11Routes.PRODUCTS) {
            ...
            if (store != null) {
                val storeId = StoreId(store)
                val vm = viewModel<ProductsViewModel>(key = "products-${storeId.value}", factory = vmFactory { ProductsViewModel(dependencies) })
                val state by vm.state.collectAsState()
                LaunchedEffect(storeId) { vm.load(storeId) }
                ProductsScreen(
                    state = state,
        ...
        composable(E11Routes.PRODUCT_DETAIL, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { entry ->
            ...
            if (store != null && id != null) {
                val storeId = StoreId(store)
                val vm = viewModel<ProductDetailViewModel>(key = "product-detail-${storeId.value}-$id", factory = vmFactory { ProductDetailViewModel(dependencies) })
                val state by vm.state.collectAsState()
                LaunchedEffect(storeId, id) { vm.load(storeId, EntityId(id)) }
        ...
        composable(E11Routes.PRODUCT_EDIT, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { entry -> val store = activeStore; val id = entry.arguments?.getString(E11Routes.PRODUCT_ID); if (store != null && id != null) ProductEditorRoute(dependencies, StoreId(store), id, { navController.popBackStack() }, { navController.popBackStack() }) }
        composable(E11Routes.VARIATIONS, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { entry -> val store = activeStore; val id = entry.arguments?.getString(E11Routes.PRODUCT_ID); if (store != null && id != null) { val storeId = StoreId(store); val vm = viewModel<VariationsViewModel>(key = "variations-${storeId.value}-$id", factory = vmFactory { VariationsViewModel(dependencies) }); val state by vm.state.collectAsState(); LaunchedEffect(storeId, id) { vm.load(storeId, EntityId(id)) }; val list = (state as? FeatureUiState.Success)?.value.orEmpty().map { variation -> VariationUiModel(variation.id.value, variation.attributes.joinToString(" · ") { "${it.name}: ${it.option}" }, variation.pricing.sale ?: variation.pricing.regular ?: "—", variation.stock?.quantity?.toString()?.removeSuffix(".0") ?: "—") }; VariationsManagementScreen(list, { navController.navigate(E11Routes.variationNew(id)) }, { variationId -> navController.navigate(E11Routes.variationEdit(id, variationId)) }) } }
        composable(E11Routes.VARIATION_NEW, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { entry -> val store = activeStore; val id = entry.arguments?.getString(E11Routes.PRODUCT_ID); if (store != null && id != null) VariationEditorRoute(dependencies, StoreId(store), id, null, { navController.popBackStack() }, { navController.popBackStack() }) }
        ...
        composable(E11Routes.SETTINGS) { val store = activeStore; if (store != null) SettingsScreen(storeName = store, storeId = StoreId(store), dependencies = dependencies, onBack = { navController.popBackStack() }, onOpenChatGpt = { navController.navigate(E11Routes.AI_CHATGPT) }, onDisconnect = { dependencies.onStoreDisconnected(); activeStore = null; navController.navigate(E11Routes.CONNECTION) { popUpTo(E11Routes.SETTINGS) { inclusive = true } } }) }
    }
}
...
private fun mapOrdersState(state: FeatureUiState<List<Order>>, hasMore: Boolean): OrdersUiState = when (state) {
    ...
    is FeatureUiState.Conflict -> OrdersUiState.Error("تعارض در وضعیت سفارش وجود دارد.", false)
    is FeatureUiState.Success -> OrdersUiState.Content(state.value.map { order -> OrderRowUiModel(order.number, order.customer?.name.orEmpty(), order.customer?.email.orEmpty(), order.status.name, formatMoney(order.total), order.payment?.methodTitle.orEmpty(), order.modifiedAt?.toString().orEmpty()) }, hasMore)
}
...
private fun mapOrderDetailState(state: FeatureUiState<Order>): OrderDetailUiState = when (state) {
    ...
    is FeatureUiState.Conflict -> OrderDetailUiState.Error("تعارض در وضعیت سفارش.")
    is FeatureUiState.Success -> OrderDetailUiState.Content(state.value)
}

private fun mapSyncState(state: FeatureUiState<SyncMetadata>): SyncUiState = when (state) {
    FeatureUiState.Loading, FeatureUiState.Pending -> SyncUiState.Running
    FeatureUiState.Empty -> SyncUiState.Idle
    is FeatureUiState.Success -> SyncUiState.Success("وضعیت همگام‌سازی فروشگاه با موفقیت دریافت شد.")
    is FeatureUiState.Error -> SyncUiState.Error(state.message)
    FeatureUiState.Offline -> SyncUiState.Error("فروشگاه در حالت آفلاین در دسترس نیست")
    is FeatureUiState.Conflict -> SyncUiState.Error("تعارض در وضعیت همگام‌سازی.")
}