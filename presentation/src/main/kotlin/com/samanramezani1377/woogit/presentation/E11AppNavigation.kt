...
internal fun E11AppNavigation(dependencies: V1PresentationDependencies, initialOrderId: String?) {
    ...
    NavHost(navController, startDestination) {
        ...
        composable(E11Routes.DASHBOARD) {
            ...
            if (s == null) LaunchedEffect(Unit) { navController.navigate(E11Routes.CONNECTION) }
            else {
        ...
        composable(E11Routes.ORDERS) {
            ...
            if (s != null) {
                ...
                val st by vm.state.collectAsState(); LaunchedEffect(s) { vm.load(s) }
                OrdersScreen(mapOrdersState(st), { id -> navController.navigate(E11Routes.order(id)) }, { vm.load(s) }, { vm.nextPage(s) }, { vm.load(s, it, true) })
            }
        }
        composable(E11Routes.ORDER_DETAIL, listOf(navArgument(E11Routes.ORDER_ID) { type = NavType.StringType })) { e ->
            val s = activeStore?.let(::StoreId); val id = e.arguments?.getString(E11Routes.ORDER_ID)
            if (s != null && id != null) { val vm = viewModel<OrderDetailViewModel>(factory = vmFactory { OrderDetailViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s, id) { vm.load(s, EntityId(id)) }; OrderDetailScreen(mapOrderDetailState(st), { vm.load(s, EntityId(id)) }, { navController.popBackStack() }, {}) }
        }
        composable(E11Routes.PRODUCTS) {
            val s = activeStore?.let(::StoreId)
            if (s != null) { val vm = viewModel<ProductsViewModel>(factory = vmFactory { ProductsViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s) { vm.load(s) }; ProductsScreen(st, { id -> navController.navigate(E11Routes.product(id)) }, { vm.load(s) }, { vm.nextPage(s) }, { vm.load(s, it, true) }, { navController.navigate(E11Routes.PRODUCT_NEW) }) }
        }
        composable(E11Routes.PRODUCT_DETAIL, listOf(navArgument(E11Routes.PRODUCT_ID) { type = NavType.StringType })) { e ->
            val s = activeStore?.let(::StoreId); val id = e.arguments?.getString(E11Routes.PRODUCT_ID)
            if (s != null && id != null) { val vm = viewModel<ProductDetailViewModel>(factory = vmFactory { ProductDetailViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s, id) { vm.load(s, EntityId(id)) }; when (val cur = st) { is FeatureUiState.Success -> ProductDetailScreen(cur.value, { navController.popBackStack() }, { navController.navigate("products/$id/edit") }, { navController.navigate(E11Routes.variations(id)) }); FeatureUiState.Loading, FeatureUiState.Pending -> GlassScaffold { GlassLoading("در حال بارگذاری محصول…") }; FeatureUiState.Empty -> GlassScaffold { GlassEmptyState("محصول پیدا نشد") }; is FeatureUiState.Error -> GlassScaffold { GlassErrorState(cur.message) }; FeatureUiState.Offline -> GlassScaffold { GlassOfflineState() }; is FeatureUiState.Conflict -> GlassScaffold { GlassErrorState("تعارض در داده‌های محصول وجود دارد") } } }
        }
        ...
        composable(E11Routes.SYNC) { val s = activeStore?.let(::StoreId); if (s != null) { val vm = viewModel<SyncViewModel>(factory = vmFactory { SyncViewModel(dependencies) }); val st by vm.state.collectAsState(); LaunchedEffect(s) { vm.load(s) }; val currentState = st; val ui = when (currentState) { FeatureUiState.Loading -> SyncUiState.Idle; FeatureUiState.Pending -> SyncUiState.Running; is FeatureUiState.Success -> SyncUiState.Success("همگام‌سازی با موفقیت انجام شد."); is FeatureUiState.Error -> SyncUiState.Error(currentState.message); else -> SyncUiState.Idle }; SyncScreen(ui, { vm.sync(s) }, { vm.sync(s) }) } }
