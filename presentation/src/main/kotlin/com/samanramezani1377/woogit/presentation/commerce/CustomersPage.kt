...
internal fun CustomersPage(storeId: StoreId, state: CommerceUiState) {
    ...
    suspend fun loadFirstPage() {
        ...
        loading = true; error = null; page = 1
    ...
    LaunchedEffect(storeId, selectedId) {
        ...
        when (val result = loader(storeId, EntityId(id))) { is CoreResult.Success -> selected = result.value; is CoreResult.Failure -> Unit }
    }
    fun loadMore() { if (loadingMore || !hasMore) return; scope.launch { loadingMore = true; val nextPage = page + 1; when (val result = CustomerRuntime.listLoader?.invoke(storeId, nextPage, CUSTOMER_PAGE_SIZE, query.trim().takeIf { it.isNotBlank() })) { is CoreResult.Success -> { customers = (customers + result.value).distinctBy { it.id?.value ?: "${it.name}:${it.email}" }; page = nextPage; hasMore = result.value.size == CUSTOMER_PAGE_SIZE }; is CoreResult.Failure -> error = result.error.customerMessage(); null -> error = "سرویس مدیریت مشتریان آماده نیست." }; loadingMore = false } }
    fun saveCustomer(customer: Customer, isNew: Boolean) { scope.launch { operationLoading = true; operationMessage = null; val result = if (isNew) CustomerRuntime.createCustomer?.invoke(storeId, customer) else { val id = customer.id; if (id == null) CoreResult.Failure(DomainError.Validation("شناسه مشتری نامعتبر است.")) else CustomerRuntime.updateCustomer?.invoke(storeId, id, customer) }; when (result) { is CoreResult.Success -> { val saved = result.value; customers = if (isNew) listOf(saved) + customers else customers.map { if (it.id == saved.id) saved else it }; selected = if (selected?.id == saved.id) saved else selected; selectedId = saved.id?.value; editorCustomer = null; showCreate = false; operationMessage = if (isNew) "مشتری با موفقیت ایجاد شد." else "اطلاعات مشتری با موفقیت ذخیره شد."; if (isNew) hasMore = customers.size < CUSTOMER_PAGE_SIZE }; is CoreResult.Failure -> operationMessage = result.error.customerMessage(); null -> operationMessage = "سرویس مدیریت مشتریان آماده نیست." }; operationLoading = false } }
    fun deleteCustomer(customer: Customer) { scope.launch { operationLoading = true; operationMessage = null; val id = customer.id; val result = if (id == null) CoreResult.Failure(DomainError.Validation("شناسه مشتری نامعتبر است.")) else CustomerRuntime.deleteCustomer?.invoke(storeId, id); when (result) { is CoreResult.Success -> { customers = customers.filterNot { it.id == customer.id }; if (selectedId == customer.id?.value) { selectedId = null; selected = null }; deleteTarget = null; operationMessage = "مشتری با موفقیت حذف شد." }; is CoreResult.Failure -> operationMessage = result.error.customerMessage(); null -> operationMessage = "سرویس مدیریت مشتریان آماده نیست." }; operationLoading = false } }
...
@Composable private fun CustomerDetailsCard(customer: Customer, orders: List<Order>, onEdit: () -> Unit, onDelete: () -> Unit, onClose: () -> Unit) { val orderCount = if (orders.isNotEmpty()) orders.size else customer.ordersCount; val totalSpent = if (orders.isNotEmpty()) orders.sumOf { it.total?.toDoubleOrNull() ?: 0.0 }.toDisplayAmount() else customer.totalSpent; val currency = orders.firstOrNull { !it.currency.isNullOrBlank() }?.currency; val amountText = if (!currency.isNullOrBlank()) "$totalSpent $currency" else totalSpent; Section("پرونده مشتری", "اطلاعات حساب، تماس، آدرس‌ها و سابقه خرید") { Text(customer.name.ifBlank { "مشتری بدون نام" }, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold); InfoLine("ایمیل", customer.email ?: "ثبت نشده"); InfoLine("تلفن", customer.phone ?: "ثبت نشده"); InfoLine("نام کاربری", customer.username ?: "ثبت نشده"); InfoLine("نقش", customer.role ?: "مشتری"); InfoLine("تعداد سفارش", orderCount.toString()); InfoLine("مجموع خرید", amountText); AddressSummary("آدرس صورتحساب", customer.billing); AddressSummary("آدرس ارسال", customer.shipping); InfoLine("سفارش‌های محلی", orders.size.toString()); if (orders.isNotEmpty()) { Text("سابقه سفارش‌ها", fontWeight = FontWeight.SemiBold); orders.take(10).forEach { order -> OrderSummary(order) }; if (orders.size > 10) Text("۱۰ سفارش اخیر نمایش داده شد.", color = GlassTokens.muted, style = MaterialTheme.typography.bodySmall) } else Text("هنوز سفارشی برای این مشتری در داده‌های اپ موجود نیست.", color = GlassTokens.muted); Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { GlassOutlinedButton("ویرایش", onEdit, modifier = Modifier.weight(1f)); GlassOutlinedButton("حذف", onDelete, modifier = Modifier.weight(1f)) }; GlassOutlinedButton("بستن", onClose, modifier = Modifier.fillMaxWidth()) } }
...
@Composable private fun CustomerEditorDialog(initial: Customer, title: String, saving: Boolean, onDismiss: () -> Unit, onSave: (Customer) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text(title) }, text = { Column(Modifier.widthIn(max = 620.dp).heightIn(max = 620.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) { LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) { item { Text("اطلاعات اصلی", fontWeight = FontWeight.Bold) }; item { FormField("نام", firstName) { firstName = it } }; item { FormField("نام خانوادگی", lastName) { lastName = it } }; item { FormField("ایمیل", email) { email = it } }; item { FormField("نام کاربری", username) { username = it } }; item { FormField("نقش", role) { role = it } }; item { FormField("تلفن", phone) { phone = it; billing = billing.copy(phone = it.takeIf(String::isNotBlank)) } }; item { Text("آدرس صورتحساب", fontWeight = FontWeight.Bold) }; item { FormField("خیابان و پلاک", billing.address1.orEmpty()) { billing = billing.copy(address1 = it) } }; item { FormField("شهر", billing.city.orEmpty()) { billing = billing.copy(city = it) } }; item { FormField("استان", billing.state.orEmpty()) { billing = billing.copy(state = it) } }; item { FormField("کدپستی", billing.postcode.orEmpty()) { billing = billing.copy(postcode = it) } }; item { FormField("کشور", billing.country.orEmpty()) { billing = billing.copy(country = it) } }; item { Text("آدرس ارسال", fontWeight = FontWeight.Bold) }; item { FormField("خیابان و پلاک", shipping.address1.orEmpty()) { shipping = shipping.copy(address1 = it) } }; item { FormField("شهر", shipping.city.orEmpty()) { shipping = shipping.copy(city = it) } }; item { FormField("استان", shipping.state.orEmpty()) { shipping = shipping.copy(state = it) } }; item { FormField("کدپستی", shipping.postcode.orEmpty()) { shipping = shipping.copy(postcode = it) } }; item { FormField("کشور", shipping.country.orEmpty()) { shipping = shipping.copy(country = it) } }; formError?.let { message -> item { Text(message, color = MaterialTheme.colorScheme.error) } } } } }, confirmButton = { Button(enabled = !saving, onClick = { val normalizedEmail = email.trim().takeIf { it.isNotBlank() }; if (firstName.isBlank() && lastName.isBlank() && normalizedEmail.isNullOrBlank()) formError = "نام یا ایمیل مشتری را وارد کنید." else { val composedName = listOf(firstName.trim(), lastName.trim()).filter { it.isNotBlank() }.joinToString(" "); onSave(initial.copy(name = composedName.ifBlank { normalizedEmail ?: "مشتری" }, email = normalizedEmail, username = username.trim().takeIf { it.isNotBlank() }, role = role.trim().ifBlank { "customer" }, phone = phone.trim().takeIf { it.isNotBlank() }, billing = billing.copy(firstName = firstName.trim().takeIf { it.isNotBlank() }, lastName = lastName.trim().takeIf { it.isNotBlank() }, phone = phone.trim().takeIf { it.isNotBlank() }), shipping = shipping.copy(firstName = firstName.trim().takeIf { it.isNotBlank() }, lastName = lastName.trim().takeIf { it.isNotBlank() }))) } }) { Text(if (saving) "در حال ذخیره…" else "ذخیره") } }, dismissButton = { TextButton(onClick = onDismiss, enabled = !saving) { Text("انصراف") } })
}
