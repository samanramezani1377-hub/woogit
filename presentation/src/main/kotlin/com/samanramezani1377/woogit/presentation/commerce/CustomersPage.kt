...
internal fun CustomersPage(storeId: StoreId, state: CommerceUiState) {
    ...
    Box(Modifier.fillMaxSize()) {
        FeatureBody {
            // Extra top breathing room keeps the first scrollable content clear of the fixed search surface.
...
@Composable private fun InfoLine(label: String, value: String) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text(label, color = GlassTokens.muted); Text(value, fontWeight = FontWeight.Medium) } }
@Composable private fun AddressSummary(title: String, address: Address?) { if (address == null) return; Section(title) { listOf(address.firstName, address.lastName, address.company, address.address1, address.address2, address.city, address.state, address.postcode, address.country, address.email, address.phone).filter { !it.isNullOrBlank() }.forEach { Text(it ?: "", color = GlassTokens.muted, style = MaterialTheme.typography.bodySmall) } } }
@Composable private fun OrderSummary(order: Order) { GlassCard(Modifier.fillMaxWidth()) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("#${order.id.value}", fontWeight = FontWeight.SemiBold); Text(order.status.name, color = GlassTokens.muted); Text(order.total ?: "0", fontWeight = FontWeight.SemiBold) } } }
