...
private fun CustomerEditorDialog(initial: Customer, title: String, saving: Boolean, onDismiss: () -> Unit, onSave: (Customer) -> Unit) {
    ...
    AlertDialog(
        ...
        text = {
            LazyColumn(
                modifier = Modifier.widthIn(max = 620.dp).heightIn(max = 620.dp),
        ...
        confirmButton = {
            Button(
                onClick = {
                    ...
                    val validEmail = email.isNotBlank() && email.contains("@")
                    when {
                        !validName -> formError = "نام مشتری الزامی است."
                        !validEmail -> formError = "ایمیل معتبر وارد کنید."
                        else -> {
                            formError = null
                            onSave(
                                initial.copy(
...
@Composable
private fun AddressFields(address: Address, onChange: (Address) -> Unit) {
...
private fun AddressSummary(title: String, address: Address?) {
    ...
    if (address == null || listOf(address.address1, address.address2, address.city, address.state, address.postcode, address.country).all { it.isNullOrBlank() }) {
        ...
        Text(
            listOf(address.address1, address.address2, address.city, address.state, address.postcode, address.country).filter { !it.isNullOrBlank() }.joinToString("، "),
