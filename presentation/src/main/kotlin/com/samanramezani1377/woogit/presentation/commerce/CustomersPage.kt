...
@Composable private fun CustomerEditorDialog(initial: Customer, title: String, saving: Boolean, onDismiss: () -> Unit, onSave: (Customer) -> Unit) { var name by remember(initial) { mutableStateOf(initial.name) }; var email by remember(initial) { mutableStateOf(initial.email.orEmpty()) }; var phone by remember(initial) { mutableStateOf(initial.phone.orEmpty()) }; var city by remember(initial) { mutableStateOf(initial.billing?.city.orEmpty()) }; AlertDialog(onDismissRequest = onDismiss, title = { Text(title) }, text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedTextField(name, { name = it }, label = { Text("نام") }, singleLine = true); OutlinedTextField(email, { email = it }, label = { Text("ایمیل") }, singleLine = true); OutlinedTextField(phone, { phone = it }, label = { Text("تلفن") }, singleLine = true); OutlinedTextField(city, { city = it }, label = { Text("شهر") }, singleLine = true) } }, confirmButton = { Button(onClick = { onSave(initial.copy(name = name.trim(), email = email.trim().takeIf { it.isNotBlank() }, phone = phone.trim().takeIf { it.isNotBlank() }, billing = initial.billing?.copy(city = city.trim()) ?: Address(city = city.trim()))) }, enabled = !saving && name.isNotBlank()) { Text(if (saving) "در حال ذخیره…" else "ذخیره") } }, dismissButton = { TextButton(onClick = onDismiss, enabled = !saving) { Text("انصراف") } }) }

private fun DomainError.customerMessage(): String = when (this) { is DomainError.Validation -> message ?: "اطلاعات مشتری نامعتبر است." else -> "عملیات مشتری انجام نشد." }
