...
class AppComposition(context:Context){
 ...
 val storeRepository=StoreRepositoryImpl(storeLocal,secure,network.httpClient);val orderRepository=OrderRepositoryImpl(orderLocal,provider,pending);val productRepository=ProductRepositoryImpl(productLocal,provider,pending);private val executor=WooCommerceOperationExecutor(db,provider,orderLocal,productLocal);val syncEngine=SyncEngine(db,executor);val syncRepository=SyncRepositoryImpl(db,syncEngine,pending)
