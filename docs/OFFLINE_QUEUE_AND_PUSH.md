...
علاوه بر این، یک نشانگر کوچک و همیشه‌قابل‌مشاهده‌ی وضعیت اتصال/سنک (Status Dot) بالای صفحه قرار دارد: سبز = متصل و همگام، زرد = اتصال ضعیف یا در حال تلاش مجدد، قرمز = قطع یا ناموفق. خطاهای شبکه موقت و خطای سرور (5xx) به‌صورت خاموش در پس‌زمینه Retry می‌شوند و کاربر با پیام مزاحم مواجه نمی‌شود؛ فقط خطاهای مهم مانند خطای احراز هویت با پیام کوتاه + دکمه تلاش مجدد نمایش داده می‌شوند. خطای اعتبارسنجی فرم (Validation) بلافاصله زیر همان فیلد مشکل‌دار نمایش داده می‌شود. خطای نبود دسترسی (Permission) با پنهان کردن کنترل مربوطه مدیریت می‌شود، بدون پیام جداگانه.
