...
class PendingOperationRepositoryImpl(private val db: WooGitDatabase) : PendingOperationRepository {
    override suspend fun getPending(storeId: StoreId): CoreResult<List<PendingOperation>> = CoreResult.Success(
        db.syncQueries.selectPendingByStore(storeId.value, System.currentTimeMillis()).executeAsList().map {
            PendingOperation(
                ...
                OperationType.valueOf(it.operation_type),
                it.payload_json,
                it.payload_hash,
                it.retry_count.toInt(),
                it.claimed_at?.let(Instant::fromEpochMilliseconds),
                it.next_attempt_at?.let(Instant::fromEpochMilliseconds),
            )
        }
    )
    ...
    override suspend fun enqueue(operation: PendingOperation): CoreResult<Unit> = runCatching {
        ...
        db.syncQueries.insert(
            ...
            operation.lastAttemptAt?.toEpochMilliseconds(),
            null,
            now,
            now,
        )
    ...
    override suspend fun markFailed(operationId: EntityId, error: String): CoreResult<Unit> =
        update(operationId, "FAILED", 0, null, error)
