    ...
    if (statusCode !in 200..299) {
        technicalErrorReporter.report(
            TechnicalErrorContext(
                feature = feature,
                location = "RepositoryResultMapper.mapResult",
                operation = operation,
                type = "HttpError",
                ...
                responseBody = body,
                details = "WooCommerce/WordPress API returned a non-success HTTP response",
            )
        )
    }
            ...
            .getOrElse { throwable ->
                technicalErrorReporter.report(
                    TechnicalErrorContext(
                        ...
                        httpStatus = statusCode.toString(),
                        responseBody = body,
                        details = "Successful response could not be parsed",
                    ), throwable
                )
                CoreResult.Failure(DomainError.Unknown("پاسخ موفق فروشگاه قابل پردازش نیست."))
            }
