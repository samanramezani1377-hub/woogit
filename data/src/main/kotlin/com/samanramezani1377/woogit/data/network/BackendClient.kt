...
class BackendClient(
    ...
    suspend fun verifySite(storeId: String, siteUrl: String, pair: CredentialPair): Result<BackendVerifyResult> = runCatching {
        ...
        val token = obj["session"]?.jsonPrimitive?.contentOrNull
            ?: throw BackendProtocolException("Missing Backend session")
    ...
    suspend fun forward(
        ...
        val key = if (normalizedMethod in MUTATION_METHODS) {
            idempotencyKey ?: stableMutationKey(storeId, normalizedMethod, path, query, body)
        } else null
    ...
    suspend fun forwardBinary(
        ...
        val response = httpClient.request(URLBuilder(url("/wp-json/woogit/v1/forward")).apply {
            ...
            header(HttpHeaders.ContentDisposition, "attachment; filename=\"${fileName.substringAfterLast('/').substringAfterLast('\\')}\"")
        ...
        return ApiResponse(
            ...
            normalizedMethod,
            path,
            response.headers.entries().associate { it.key.lowercase() to it.value.joinToString(",") },
        )
    }

    suspend fun revokeSession(storeId: String): Result<Unit> = runCatching {
        val token = sessions.get(storeId) ?: return@runCatching Unit
        val response = httpClient.post(url("/wp-json/woogit/v1/sessions/revoke")) {
            header("X-WooGit-App-Version", appVersion)
            header("X-WooGit-Session", token)
        }
        val body = response.bodyAsText()
    ...
    private fun stableMutationKey(
        ...
        val canonicalQuery = query.toSortedMap().entries.joinToString("&") { "${it.key}=${it.value}" }
        val canonical = "$storeId|$method|$path|$canonicalQuery|${body.orEmpty()}"
