...
@Serializable private data class VariationCache(val id: String,val product_id: String,val sku: String? = null,val regular_price: String? = null,val sale_price: String? = null,val stock_quantity: Double? = null,val stock_status: String = "instock",val manage_stock: Boolean = false,val image: ImageCache? = null,val attributes: List<VarAttrCache> = emptyList(),val date_modified_gmt: String? = null)
...
class SqlVariationDataSource(private val db: WooGitDatabase) : LocalVariationDataSource {
 override fun list(storeId:StoreId,productId:EntityId):CoreResult<List<Variation>>=CoreResult.Success(db.variationQueries.selectVariationsByProduct(productId.value,storeId.value).executeAsList().map{mapVariation(it.payload_json)})
 override fun get(storeId:StoreId,productId:EntityId,id:EntityId):CoreResult<Variation>=db.variationQueries.selectVariationById(id.value,productId.value,storeId.value).executeAsOneOrNull()?.let{CoreResult.Success(mapVariation(it.payload_json))}?:CoreResult.Failure(DomainError.NotFound("variation",id.value))
 private fun mapVariation(payload:String):Variation{val v=localJson.decodeFromString<VariationCache>(payload);return Variation(EntityId(v.id),EntityId(v.product_id),v.attributes.map{VariationAttribute(it.name,it.option)},Pricing(v.regular_price,v.sale_price,v.sale_price!=null),Stock(v.stock_quantity,when(v.stock_status){"outofstock"->StockStatus.OUT_OF_STOCK;"onbackorder"->StockStatus.ON_BACKORDER;else->StockStatus.IN_STOCK},v.manage_stock),v.sku,v.image?.let{im->ProductImage(im.id?.let{id->EntityId(id.toString())},im.src,im.name,im.alt)},v.date_modified_gmt?.let(Instant::parse))}
 override fun upsert(storeId:StoreId,value:Variation):CoreResult<Unit>{val payload=localJson.encodeToString(VariationCache(value.id.value,value.productId.value,value.sku,value.pricing.regular,value.pricing.sale,value.stock?.quantity,value.stock?.status?.name?.lowercase()?:"instock",value.stock?.manageStock?:false,value.image?.let{ImageCache(it.id?.value?.toLongOrNull(),it.src,it.name,it.alt)},value.attributes.map{VarAttrCache(it.name,it.option)},value.modifiedAt?.toString()));val now=System.currentTimeMillis();db.variationQueries.upsertVariation(value.id.value,value.productId.value,storeId.value,value.stock?.status?.name?:"",value.pricing.regular?.toLongOrNull(),value.stock?.quantity?.toLong(),payload,value.modifiedAt?.toString()?:"",value.modifiedAt?.toString()?:"",now,now);return CoreResult.Success(Unit)}
