...
class SqlVariationDataSource(private val db: WooGitDatabase) : LocalVariationDataSource {
    ...
    override fun upsert(storeId: StoreId, value: Variation): CoreResult<Unit> { val payload = localJson.encodeToString(VariationCache(value.id.value.toLongOrNull() ?: 0L, value.productId.value.toLongOrNull() ?: 0L, value.sku, value.pricing.regular, value.pricing.sale, value.stock?.quantity, value.stock?.status?.name?.lowercase(), value.stock?.manageStock, value.image?.let { ImageCache(it.id?.value?.toLongOrNull(), it.src, it.name, it.alt) }, value.attributes.map { VarAttrCache(it.name, it.option) }, value.modifiedAt?.toString())); val now = System.currentTimeMillis(); db.variationQueries.upsertVariation(value.id.value, value.productId.value, storeId.value, value.stock?.status?.name ?: "", value.pricing.regular?.toLongOrNull(), value.stock?.quantity?.toLong(), payload, value.modifiedAt?.toString() ?: "", value.modifiedAt?.toString() ?: "", now, now); return CoreResult.Success(Unit) }
