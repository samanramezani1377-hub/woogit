...
private fun <T> Result<T>.core(): CoreResult<T> = fold({ CoreResult.Success(it) }, { CoreResult.Failure(it.domainError()) })
...
class WooCommerceClientProvider(private val db:WooGitDatabase, private val secureStore:SecureCredentialStore, private val httpClient:io.ktor.client.HttpClient){
 suspend fun client(storeId:StoreId):Result<Pair<StoreConnection,TypedWooCommerceApi>>=runCatching{
  val store=db.storeQueries.selectById(storeId.value).executeAsOneOrNull()?:error("Store not found")
...
class OrderRepositoryImpl(private val local:LocalOrderDataSource<Order>,private val storeLocal:LocalStoreDataSource<StoreConnection>,private val provider:WooCommerceClientProvider,private val pending:PendingOperationRepository):OrderRepository{
 override suspend fun get(storeId:StoreId,id:EntityId):CoreResult<Order>{val cached=local.get(storeId,id);return when(cached){is CoreResult.Success->{provider.client(storeId).fold({(_,api)->api.order("",id.value.toLong()).core()},{cached}) .let{if(it is CoreResult.Success){local.upsert(storeId,it.value);it}else cached}};is CoreResult.Failure->{provider.client(storeId).fold({(store,api)->api.order(store.baseUrl,id.value.toLong()).fold({o->local.upsert(storeId,o.domain());CoreResult.Success(o.domain())},{it.domainError().let(CoreResult::Failure)})},{cached})}}}
 override suspend fun list(storeId:StoreId,page:Int,perPage:Int,search:String?,status:String?):CoreResult<List<Order>>{val cached=if(page==1)local.list(storeId) else null;val remote=provider.client(storeId);return remote.fold({(store,api)->api.orders(store.baseUrl,page,perPage,search,status).fold({items->val domains=items.map{it.domain()};domains.forEach{local.upsert(storeId,it)};CoreResult.Success(domains)}, {cached?:CoreResult.Failure(it.domainError())})},{cached?:CoreResult.Failure(it.domainError())})}
 override suspend fun update(storeId:StoreId,id:EntityId,order:Order):CoreResult<Order>{local.upsert(storeId,order);val payload=json.encodeToString(OrderWrite(order.status.name.lowercase()));val op=PendingOperation(EntityId("order-${storeId.value}-$id-${hash(payload).take(16)}"),storeId,"order",id,OperationType.UPDATE,payload,hash(payload),0,null);pending.enqueue(op);return provider.client(storeId).fold({(store,api)->api.updateOrder(store.baseUrl,id.value.toLong(),WooOrderTypedDto(id.value.toLong(),status=order.status.name.lowercase())).fold({remote->local.upsert(storeId,remote.domain());pending.markSucceeded(op.id);CoreResult.Success(remote.domain())},{err->if(err.domainError().recoverable)CoreResult.Success(order) else CoreResult.Failure(err.domainError())})},{CoreResult.Success(order)})}
}
...
class ProductRepositoryImpl(private val local:LocalProductDataSource<Product>,private val provider:WooCommerceClientProvider,private val pending:PendingOperationRepository):ProductRepository{
 override suspend fun get(storeId:StoreId,id:EntityId)=provider.client(storeId).fold({(store,api)->api.product(store.baseUrl,id.value.toLong()).fold({remote->val d=remote.domain();local.upsert(storeId,d);CoreResult.Success(d)},{err->local.get(storeId,id).let{if(it is CoreResult.Success)it else CoreResult.Failure(err.domainError())}})},{local.get(storeId,id)})
 override suspend fun list(storeId:StoreId,page:Int,perPage:Int,search:String?)=provider.client(storeId).fold({(store,api)->api.products(store.baseUrl,page,perPage,search).fold({items->val d=items.map{it.domain()};d.forEach{local.upsert(storeId,it)};CoreResult.Success(d)},{err->local.list(storeId).let{if(it is CoreResult.Success)it else CoreResult.Failure(err.domainError())}})},{local.list(storeId)})
 override suspend fun create(storeId:StoreId,product:Product):CoreResult<Product>{local.upsert(storeId,product);val payload=json.encodeToString(ProductWrite(product.name,product.sku,product.description,product.shortDescription,product.status.name.lowercase(),product.type.name.lowercase(),product.pricing.regular,product.pricing.sale,images=product.images.map{WooImageTypedDto(it.id?.value?.toLong(),it.src,it.name,it.alt)},attributes=product.attributes.map{WooProductAttributeDto(it.id?.value?.toLong(),it.name,it.visible,it.variation,it.options)}));val op=PendingOperation(EntityId("product-create-${storeId.value}-${hash(payload).take(20)}"),storeId,"product",product.id,OperationType.CREATE,payload,hash(payload),0,null);pending.enqueue(op);return provider.client(storeId).fold({(store,api)->api.createProduct(store.baseUrl,WooProductTypedDto(product.id.value.toLong(),product.name,sku=product.sku,description=product.description,short_description=product.shortDescription,status=product.status.name.lowercase(),type=product.type.name.lowercase(),regular_price=product.pricing.regular,sale_price=product.pricing.sale,images=product.images.map{WooImageTypedDto(it.id?.value?.toLong(),it.src,it.name,it.alt)},attributes=product.attributes.map{WooProductAttributeDto(it.id?.value?.toLong(),it.name,it.visible,it.variation,it.options)})).fold({r->val d=r.domain();local.upsert(storeId,d);pending.markSucceeded(op.id);CoreResult.Success(d)},{CoreResult.Success(product)})},{CoreResult.Success(product)})}
