...
class SyncRepositoryImpl(private val db:WooGitDatabase,private val engine:SyncEngine,private val pending:PendingOperationRepository):SyncRepository{
 ...
 override suspend fun getConflicts(storeId:StoreId):CoreResult<List<Conflict>>=CoreResult.Success(db.syncQueries.selectConflicts(storeId.value).executeAsList().map{Conflict(EntityId(it.id),EntityId(it.entity_id),EntityVersion(it.base_snapshot?:"local",null),EntityVersion(it.server_snapshot,"".let{null}),it.conflicting_fields,null)})
