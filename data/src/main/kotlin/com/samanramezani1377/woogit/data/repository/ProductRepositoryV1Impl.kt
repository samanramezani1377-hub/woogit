...
private fun WooProductTypedDto.toDomain(settings: WooSystemStatusSettingsDto? = null) = Product(
    ...
    currency = settings?.currency?.takeIf { it.isNotBlank() }, currencySymbol = settings?.currency_symbol?.takeIf { it.isNotBlank() },
    currencyPosition = settings?.currency_position?.takeIf { it.isNotBlank() }, thousandSeparator = settings?.thousand_separator,
    decimalSeparator = settings?.decimal_separator, numberOfDecimals = settings?.number_of_decimals,
    weightUnit = settings?.weight_unit?.takeIf { it.isNotBlank() }, dimensionUnit = settings?.dimension_unit?.takeIf { it.isNotBlank() },
)
...
private suspend fun applyMissingProductFields(api: TypedWooCommerceApi, baseUrl: String, id: Long, requested: Product, initial: WooProductTypedDto, operationId: String): Result<WooProductTypedDto> { var remote = initial; expectedStatus(requested.status)?.takeIf { !it.equals(remote.status.trim(), true) }?.let { status -> remote = api.updateProductFields(baseUrl, id, buildJsonObject { put("status", status) }, "$operationId:status").getOrElse { return Result.failure(it) } }; expectedType(requested.type)?.takeIf { !it.equals(remote.type.trim(), true) }?.let { type -> remote = api.updateProductFields(baseUrl, id, buildJsonObject { put("type", type) }, "$operationId:type").getOrElse { return Result.failure(it) } }; return Result.success(remote) }
...
class ProductRepositoryV1Impl(private val local: LocalProductDataSource<Product>, private val provider: WooCommerceClientProvider, private val coordinator: MutationCoordinator, private val pending: PendingOperationRepository, private val onCacheHit: (StoreId) -> Unit = {}) : ProductRepository {
