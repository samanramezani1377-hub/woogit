...
private fun expectedType(type: ProductType): String? = when (type) { ProductType.SIMPLE -> "simple"; ProductType.GROUPED -> "grouped"; ProductType.EXTERNAL -> "external"; ProductType.OTHER -> null }
    ...
    override suspend fun create(storeId: StoreId, product: Product): CoreResult<Product> {
        ...
        return provider.client(storeId).fold({ (store, api) ->
            api.createProduct(store.baseUrl, product.toDto(operationId)).fold(
                onSuccess = { initial ->
                    applyMissingProductFields(api, store.baseUrl, initial.id, product, initial).fold(
                        onSuccess = { corrected ->
                            val refreshed = api.product(store.baseUrl, initial.id).getOrElse { corrected }
                            when (val verified = verifyProductMutation(product, refreshed)) {
                                is CoreResult.Failure -> {
                                    pending.markFailed(operation.id, verified.error.toString())
                                    CoreResult.Failure(verified.error)
                                }
                                is CoreResult.Success -> {
                                    val value = refreshed.toDomain()
                        ...
                        onFailure = { error ->
                            pending.markFailed(operation.id, error.message ?: "Product status/type correction failed")
                            CoreResult.Failure(error.productDomainError())
                        },
                    )
                },
