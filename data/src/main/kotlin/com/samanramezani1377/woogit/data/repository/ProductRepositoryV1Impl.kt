...
private fun expectedType(type: ProductType): String? = when (type) { ProductType.PUBLISHED -> "publish"; ProductType.PENDING -> "pending"; ProductStatus.PRIVATE -> "private"; ProductStatus.DRAFT -> "draft"; ProductStatus.OTHER -> null }
private fun expectedType(status: ProductType): String? = when (status) { ProductType.GROUPED -> "grouped"; ProductType.EXTERNAL -> "external"; ProductType.VARIABLE -> "variable"; ProductType.SIMPLE -> "simple"; ProductType.OTHER -> null }
...
class ProductRepositoryV1Impl(private val local: LocalProductDataSource<Product>, private val provider: WooCommerceClientProvider, private val coordinator: MutationCoordinator, private val pending: PendingOperationRepository) : ProductRepository {
    ...
    private suspend fun commerceSettings(storeId: StoreId, api: TypedWooCommerceApi, baseUrl: String): WooSystemStatusSettingsDto? {
        commerceSettingsCache[storeId.value]?.let { return it }
        return api.validate(baseUrl).getOrNull()?.settings?.also { commerceSettingsCache[storeId.value] = it }
    }

    private suspend fun enrich(storeId: StoreId, values: List<Product>): List<Product> = provider.client(storeId).fold({ (store, api) ->
        val settings = commerceSettings(storeId, api, store.baseUrl)
        if (settings == null) values else values.map { it.copy(
            currency = settings.currency.takeIf(String::isNotBlank), currencySymbol = settings.currency_symbol.takeIf(String::isNotBlank),
            currencyPosition = settings.currency_position.takeIf(String::isNotBlank), thousandSeparator = settings.thousand_separator,
            decimalSeparator = settings.decimal_separator, numberOfDecimals = settings.number_of_decimals,
            weightUnit = settings.weight_unit.takeIf(String::isNotBlank), dimensionUnit = settings.dimension_unit.takeIf(String::isNotBlank)
        ) }
    }, { values })

    override suspend fun get(storeId: StoreId, id: EntityId): CoreResult<Product> = local.get(storeId, id).fold(
        { cached -> enrich(storeId, listOf(cached)).firstOrNull()?.let { CoreResult.Success(it) } ?: CoreResult.Success(cached) },
        { provider.client(storeId).fold({ (store, api) -> api.product(store.baseUrl, id.value.toLong()).fold({ remote ->
            val settings = commerceSettings(storeId, api, store.baseUrl); val value = remote.toDomain(settings); local.upsert(storeId, value); CoreResult.Success(value)
        }, { local.get(storeId, id) }) }, { local.get(storeId, id) }) }
    )

    override suspend fun list(storeId: StoreId, page: Int, perPage: Int, search: String?): CoreResult<List<Product>> = if (page == 1 && search.isNullOrBlank()) {
        local.list(storeId).fold({ cached -> if (cached.isNotEmpty()) CoreResult.Success(enrich(storeId, cached)) else fetchList(storeId, page, perPage, search) }, { fetchList(storeId, page, perPage, search) })
    } else fetchList(storeId, page, perPage, search)
