...
object ProductRepositoryV1Mapper {
    fun toDomain(v: WooProductTypedDto): Product = Product(
        EntityId(v.id.toString()), v.name, v.sku, v.description, v.short_description,
        when (v.status.trim().lowercase()) {
        ...
        Stock(v.stock_quantity, when (v.stock_status.trim().lowercase()) { "outofstock" -> StockStatus.OUT_OF_STOCK; "onbackorder" -> StockStatus.ON_BACKORDER; else -> StockStatus.IN_STOCK }, v.manage_stock),
