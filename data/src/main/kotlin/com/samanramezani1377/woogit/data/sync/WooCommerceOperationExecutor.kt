...
class WooCommerceOperationExecutor(private val db:WooGitDatabase,private val provider:com.samanramezani1377.woogit.data.repository.WooCommerceClientProvider,private val orders:LocalOrderDataSource<com.samanramezani1377.woogit.core.domain.model.Order>,private val products:LocalProductDataSource<com.samanramezani1377.woogit.core.domain.model.Product>):OperationExecutor{
 override suspend fun execute(operation:Pending_operation){
  ...
  when(operation.entity_type to OperationType.valueOf(operation.operation_type)){
   ...
   "product" to OperationType.CREATE->{val p=json.decodeFromString<ProductMutation>(operation.payload_json);val reconciled=api.products(store.baseUrl,1,50,p.name).getOrThrow().firstOrNull{candidate->candidate.meta_data.any{it.key=="_woogit_operation_id"&&it.value?.jsonPrimitive?.contentOrNull==operation.id}};val remote=reconciled?:api.createProduct(store.baseUrl,WooProductTypedDto(0,p.name,sku=p.sku,description=p.description,short_description=p.short_description,status=p.status,type=p.type,regular_price=p.regular_price,sale_price=p.sale_price,images=p.images,attributes=p.attributes,meta_data=listOf(WooMetaDataDto(key="_woogit_operation_id",value=JsonPrimitive(operation.id)))).getOrThrow();products.delete(storeId,EntityId(operation.entity_id));products.upsert(storeId,remote.toDomain())}
